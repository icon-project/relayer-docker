...
REQUIRED_COMMANDS=("curl" "openssl" "docker" "dig")
MISSING_COMMANDS=()
...
if ! groups "$USER" | grep &>/dev/null "\bdocker\b"; then
  read -rp "User is not in the docker group. Add user to docker group? (yes/no) [no]: " ADD_DOCKER_GROUP
...
curl -sL "${REPO_URL}/${DOCKER_COMPOSE_FILE}" -o "${CONFIG_DIR}/${DOCKER_COMPOSE_FILE}"

# Prompt user for environment variables

## data directory
read -rp "Enter Data Directory [$CONFIG_DIR/.data]: " DATA_DIR
...
read -rp "Enter AWS Default Region [us-east-1]: " AWS_DEFAULT_REGION
AWS_DEFAULT_REGION=${AWS_DEFAULT_REGION:-us-east-1}

# Image Versions
read -rp "Enter Relayer Image Version [latest]: " RELAYER_IMAGE_VERSION
RELAYER_IMAGE_VERSION=${RELAYER_IMAGE_VERSION:-latest}

read -rp "Enter Dashboard Image Version [latest]: " DASHBOARD_IMAGE_VERSION
DASHBOARD_IMAGE_VERSION=${DASHBOARD_IMAGE_VERSION:-latest}

# Restart Policy
read -rp "Enter Restart Policy [unless-stopped]: " RESTART_POLICY
RESTART_POLICY=${RESTART_POLICY:-unless-stopped}

# Admin Credentials
read -rp "Enter Admin Email [admin@icon.community]: " DEFAULT_ADMIN_EMAIL
DEFAULT_ADMIN_EMAIL=${DEFAULT_ADMIN_EMAIL:-admin@icon.community}

read -rp "Enter Admin Password [p@ssw0rd]: " DEFAULT_ADMIN_PASSWORD
...
read -rp "Enable Let's Encrypt? (yes/no) [no]: " ENABLE_LETSENCRYPT
ENABLE_LETSENCRYPT=${ENABLE_LETSENCRYPT:-no}
if [[ "${ENABLE_LETSENCRYPT}" == "yes" || "${ENABLE_LETSENCRYPT}" == "y" ]]; then
  ENABLE_LETSENCRYPT=1
  read -rp "Use Let's Encrypt Staging Environment? (yes/no) [no]: " LETSENCRYPT_USE_STAGING
  ...
  read -rp "Enter Let's Encrypt Domain: " LETSENCRYPT_DOMAIN
  read -rp "Enter Let's Encrypt Email: " LETSENCRYPT_EMAIL
  ...
  LETSENCRYPT_DOMAIN=""
  LETSENCRYPT_EMAIL=""
  read -rp "Enter NEXTAUTH_URL (e.g., https://your-ip-address): " NEXTAUTH_URL
fi

# RELAYER CONFIG_PATH
read -rp "Enter relayer config (path to config.yaml) [$HOME/.centralized-relay/config.yaml]: " CONFIG_PATH
...
if [[ -d "$HOME/.centralized-relay/keystore" ]]; then
  read -rp "Copy keystore to ${CONFIG}/relayer/keystore? (yes/no) [yes]: " COPY_KEYSTORE
